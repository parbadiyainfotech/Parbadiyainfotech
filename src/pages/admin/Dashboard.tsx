...
import { ClipboardList, ArrowUpRight, Loader2, Phone, Calendar, User, Clock, CheckCircle, XCircle, Search, Filter, MessageSquare } from 'lucide-react';
...
export function Dashboard() {
  ...
  const stats = [
    { label: 'Total Inquiries', value: inquiries.length, icon: ClipboardList, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Active Requests', value: inquiries.filter(i => i.status === 'Pending' || i.status === 'In Progress').length, icon: Clock, color: 'text-orange-600', bg: 'bg-orange-50' },
    { label: 'Services Completed', value: inquiries.filter(i => i.status === 'Completed').length, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Cancelled Inquiries', value: inquiries.filter(i => i.status === 'Cancelled').length, icon: XCircle, color: 'text-red-600', bg: 'bg-red-50' }
  ];
    ...
    <div className="p-4 md:p-10 max-w-7xl mx-auto space-y-8 w-full overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage customer service requests and statuses.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white border border-slate-200 p-6 rounded-xl flex flex-col justify-between h-full relative overflow-hidden group hover:shadow-md transition-all">
             <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
               <stat.icon className="w-16 h-16 text-slate-400" />
             </div>
             <div>
               <span className="text-xs font-mono text-slate-500 uppercase tracking-wider font-semibold">{stat.label}</span>
               <div className="text-5xl font-bold text-slate-900 mt-2">{stat.value}</div>
             </div>
             <div className={cn("mt-4 flex items-center gap-1 text-sm font-medium", stat.color)}>
                ...
                {filteredInquiries.length === 0 && !loading ? (
                  <tr>
                    <td colSpan={5} className="py-20 text-center text-slate-500 font-medium">
                      {searchTerm || startDate || endDate ? 'No results matching your filters.' : 'No inquiries found. They will appear here when customers submit the form.'}
                    </td>
                  </tr>
                ) : (
